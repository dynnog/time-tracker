...
impl MeetingDetectionService {
    fn new(detector: Box<dyn NativeMeetingDetector>) -> Self {
        Self { detector, running: false, detected: false, applications: Vec::new() }
    }
    ...
    fn poll(&mut self) -> DetectionStatus {
        if !self.running { return self.status(None); }
        self.applications = self.detector.scan();
        let next = self.applications.iter().any(|application| application.confidence == "high");
...
#[cfg(target_os = "macos")]
fn platform_detector() -> Box<dyn NativeMeetingDetector> { Box::new(macos::MacMeetingDetector::new()) }
#[cfg(target_os = "windows")]
fn platform_detector() -> Box<dyn NativeMeetingDetector> { Box::new(windows::WindowsMeetingDetector::new()) }
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
fn platform_detector() -> Box<dyn NativeMeetingDetector> { Box::new(UnsupportedMeetingDetector) }
...
impl NativeMeetingDetector for UnsupportedMeetingDetector {
    ...
    fn scan(&mut self) -> Vec<DetectedApplication> { Vec::new() }
}
...
fn with_service(
    ...
    let mut service = state.lock().map_err(|_| "Meeting detector state is unavailable.".to_string())?;
    Ok(action(&mut service))
}

#[tauri::command]
pub fn meeting_detector_start(state: tauri::State<'_, Mutex<MeetingDetectionService>>) -> Result<DetectionStatus, String> {
    with_service(state, MeetingDetectionService::start)
}
#[tauri::command]
pub fn meeting_detector_stop(state: tauri::State<'_, Mutex<MeetingDetectionService>>) -> Result<DetectionStatus, String> {
    with_service(state, MeetingDetectionService::stop)
}
#[tauri::command]
pub fn meeting_detector_poll(state: tauri::State<'_, Mutex<MeetingDetectionService>>) -> Result<DetectionStatus, String> {
...
mod tests {
    ...
    struct FakeDetector { values: VecDeque<Vec<DetectedApplication>> }
    impl NativeMeetingDetector for FakeDetector {
        fn platform(&self) -> &'static str { "test" }
        fn scan(&mut self) -> Vec<DetectedApplication> { self.values.pop_front().unwrap_or_default() }
    }
    fn teams() -> DetectedApplication {
        DetectedApplication { id: "teams", name: "Microsoft Teams", confidence: "high", signal: "active microphone" }
    }

    #[test]
    fn emits_only_overall_state_transitions() {
        let detector = FakeDetector { values: VecDeque::from([vec![], vec![teams()], vec![teams()], vec![], vec![]]) };
